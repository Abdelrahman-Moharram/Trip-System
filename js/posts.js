...
var ids = [0,1,2];
var likes = [1,331,2213];
var dislikes = [10,22,31];
...
for (var i = 0; i < 3; i++){
    ...
    var likesFactor = ((likes[i] - dislikes[i]) * 100 ) / (likes[i] + dislikes[i]);
    if(likesFactor < 0){
        likesFactor += 100;
    }
...
var likesFactor = ((likes - dislikes) * 100 ) / (likes + dislikes);
...
likesBar[0].style.width = (likesFactor) + "%";
dislikesBar[0].style.width = (100 - likesFactor) + "%";
console.log(likesFactor);
...
var dislikesFactor = ((dislikes - likes) * 100 ) / (likes + dislikes);
if(dislikesFactor < 0){
    dislikesFactor += 100;
}
console.log(dislikesFactor);

var likesBar = document.getElementsByClassName("gaven-likes-counter");
var dislikesBar = document.getElementsByClassName("gaven-dislikes-counter");


dislikesBar[0].style.width = (dislikesFactor) + "%";
likesBar[0].style.width = (100 - dislikesFactor) + "%";
